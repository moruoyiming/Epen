...
import com.hero.basiclib.http.ExceptionHandle;
...
//                        .map(new HandleFuc<T>())  //这里可以取出BaseResponse中的Result
    ...
    private static class HandleFuc<T> implements Function<TimeBasicResponse<T>, T> {
        @Override
        public T apply(TimeBasicResponse<T> response) {
